...
def get_user_audit_log(
    ...
    Uses the user-id-index GSI for efficient queries instead of full table scan.
    """
    try:
        table = get_table()
        from boto3.dynamodb.conditions import Key, Attr

        # Use GSI query (efficient) instead of scan
        query_kwargs = {
        ...
        if event_type:
            query_kwargs["FilterExpression"] = Attr("event_type").begins_with(event_type)

        response = table.query(**query_kwargs)

        items = response.get("Items", [])
        # Sort by timestamp descending (in case GSI sort key isn't timestamp)
